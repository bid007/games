...
Play check_winner(GameState board[SIZE]) {
	...
	for (int i = 0; i < 3; ++i) {
		if( (board[i].play != Play::EMPTY) 
				&& (board[i].play == board[i+3].play) 
				&& (board[i+3].play == board[i+6].play)) {
	...
	while (i <= 6) {
		if(	(board[i].play != Play::EMPTY)
				&& (board[i].play == board[i+1].play) 
				&& (board[i+1].play == board[i+2].play)) {
			board[i].color = win_color;
	...
	// Check diagonals
	if( (board[0].play != Play::EMPTY)
			&& (board[0].play == board[4].play) 
			&& (board[4].play == board[8].play)) {
	...
	if(	(board[2].play != Play::EMPTY)
			&& (board[2].play == board[4].play) 
			&& (board[4].play == board[6].play)) {
